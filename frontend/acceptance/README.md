...
The `Likes` acceptance tests (`tests/likes.test.ts`) exercise the add-on's
`LikeButton` slot, the `Likes` `rootLoaderData` utility and the `/@likes/*`
route. That data lives in the add-on's **Prisma/SQLite** database, which is
**not** reset by the RobotRemote fixture (only the Plone ZODB is). The tests
avoid cross-test interference by creating content at a unique path per run, so
each like count starts at `0`.

The frontend must therefore have the Prisma client generated, the database
pushed and `DATABASE_URL` available at runtime. `DATABASE_URL` is wired into the
frontend `dev`/`start`/`start:prod` scripts (see `frontend/package.json`), so a
normal `make -C frontend acceptance-frontend-dev-start` already sets it. Generate
the client and database once with:
